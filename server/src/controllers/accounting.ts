...
const getSum = async (req: Request, res: Response) => {
  const sqlQuery = `
      SELECT SUM(IF(method = 'check', amount, 0)) AS checks,
             SUM(IF(method = 'card', amount, 0))  AS cards,
             SUM(IF(method = 'cash', amount, 0))  AS cashs
      FROM accounting
  ...
  try {
    ...
    log.message('info', 'rows: '+ rows );
    const sum = rows[0].checks + rows[0].cards + rows[0].cashs;
    log.message('info','Returned transaction sum');
